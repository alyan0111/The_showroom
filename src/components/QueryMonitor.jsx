...
function highlightSql(sql) {
  ...
  keywords.forEach((kw) => {
    ...
    highlighted = highlighted.replace(
      regex,
      `<span class="text-[#ff2d9b] font-semibold">${kw}</span>`,
    );
  });
  return highlighted;
}
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/5 bg-[#05051a] shrink-0">
            <div className="flex items-center gap-2">
              <span
                className={`w-2 h-2 rounded-full ${live ? "bg-[#00f5ff] animate-pulse" : "bg-gray-600"}`}
              />
                ...
                {logs.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-gray-500 text-sm py-10">
                    ...
                    <p className="text-xs text-gray-600 mt-1">
                      Interact with the app to see live SQL.
                    </p>
                  </div>
                ) : (
                  logs.map((log) => (
                      ...
                      className={`rounded-xl border p-3 text-xs
                        ${
                          log.error
                            ...
                            : "border-white/5 bg-[#0a0a2e] hover:border-[#00f5ff]/20"
                        } transition`}
                    >
                      <div className="flex items-center justify-between mb-1.5 flex-wrap gap-1">
                        <div className="flex items-center gap-2">
                          {log.method && (
                            <span
                              className={`px-1.5 py-0.5 rounded font-bold text-[10px] ${methodColor[log.method] || "text-gray-400 bg-white/5"}`}
                            >
                              {log.method}
                            </span>
                          )}
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <span
                            className={`font-mono ${log.duration > 100 ? "text-yellow-400" : "text-gray-500"}`}
                          >
                            {log.duration}ms
                          </span>
                      ...
                      {log.params && log.params.length > 0 && (
                        <p className="text-gray-500 mt-1.5 font-mono text-[10px]">
                          params: [
                          {log.params.map((p) => JSON.stringify(p)).join(", ")}]
                        </p>
                      )}
